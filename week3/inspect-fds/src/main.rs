...
fn main() {
    ...
    let target_process = ps_utils::get_target(&target).unwrap();
    let target_process = match target_process {
    ...
    if let Ok(children) = ps_utils::get_child_processes(target_process.pid) {
